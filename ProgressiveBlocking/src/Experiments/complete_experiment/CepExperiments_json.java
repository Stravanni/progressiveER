 ...
 * @author giovanni
 */
public class CepExperiments_json {
    ...
    public static void main(String[] args) {
        ...
        for (int dataset_num : new int[]{0}) {
            ...
            for (WeightingScheme wScheme : ws) {
                ...
                //methods[0] = new CepCnpEntities(wScheme, profiles[0].size() + (CLEAN ? profiles[1].size() : 0));
                ...
                for (CepCnp progressiveBlocking : methods) {
                    ...
                    res_final.set_t_init(Instant.now().toEpochMilli());
                    ...
                    while (progressiveBlocking.hasNext()) {
                        ...
                        if ((pc - pc_old) > .02) {
                            ...
                            /*res_final.add_res(detectedDuplicates, comparisons, Instant.now().toEpochMilli() - start.toEpochMilli());*/
                        }
        ...
        try {
            writer = new PrintWriter(file_results, "UTF-8");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
